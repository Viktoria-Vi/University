...
#reader(lib "htdp-beginner-reader.ss" "lang")((modname A2_2) (read-case-sensitive #t) (teachpacks ((lib "image.rkt" "teachpack" "2htdp") (lib "universe.rkt" "teachpack" "2htdp"))) (htdp-settings #(#t constructor repeating-decimal #f #t none #f ((lib "image.rkt" "teachpack" "2htdp") (lib "universe.rkt" "teachpack" "2htdp")) #f)))
(define-struct Zweirad (weight))
...
(define (PKW?-Light-Enough? vehicle max-punctual-weight)
  (if (and (PKW? vehicle)
          (< (PKW-weight vehicle) max-punctual-weight))
    true
      false)
)
; vehicle Number -> boolean
...
(define (ZweiAchser?-Light-Enough? vehicle max-punctual-weight)
  (if(and (ZweiAchser? vehicle)
         (< (ZweiAchser-x vehicle) max-punctual-weight)
         (< (ZweiAchser-y vehicle) max-punctual-weight))
    true
      false)
)
; vehicle Number -> boolean
...
(define(DreiAchser?-Light-Enough? vehicle max-punctual-weight)
  (if(and (DreiAchser? vehicle)
         (< (DreiAchser-x vehicle) max-punctual-weight)
         (< (DreiAchser-y vehicle) max-punctual-weight)
         (< (DreiAchser-z vehicle) max-punctual-weight))
    true
      false)
)
; vehicle Number -> boolean
...
(check-expect(darf-passieren Zweirad 200) true)
